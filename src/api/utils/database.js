...
async function createTag(email, tagTitle, durationInMinutes) {
    const text =
        "INSERT INTO tags (id, email, title, duration_in_minutes) VALUES (default, $1, $2, $3) RETURNING id;";
...
async function updateTag(email, tagId, tagTitle, durationInMinutes) {
    const text =
        "UPDATE tags SET title = $3, duration_in_minutes = $4 WHERE email = $1 AND id = $2;";
...
async function getAllEvents(email) {
    const text =
        "SELECT id,title,tag_id,description,start_time,end_time FROM events WHERE email = $1 ;";
    const values = [email];
    return client.query(text, values);
}
...
async function createEvent(email, title, tag_id, description, start_time, end_time) {
    const text =
        "INSERT INTO events(id,email,title,tag_id,description,start_time,end_time) VALUES(default,$1,$2,$3,$4,$5,$6) RETURNING id;";
...
async function updateEvent(event_id, email, title, tag_id, description, start_time, end_time) {
    const text =
        "UPDATE events SET title = $3,tag_id = $4,description = $5,start_time=$6,end_time=$7 WHERE email = $2 AND id = $1;";
...
async function getAllTasks(email) {
    const text =
        "SELECT id,title,tag_id,description,start_time,end_time,completed FROM tasks WHERE email = $1 ;";
    const values = [email];
    return client.query(text, values);
}
...
async function createTask(email, title, tag_id, description, start_time, end_time, completed) {
    const text =
        "INSERT INTO tasks(id,email,title,tag_id,description,start_time,end_time,completed) VALUES(default,$1,$2,$3,$4,$5,$6,$7) RETURNING id;";
